...
namespace Catch {
    ...
    public:
        struct FilterMatch {
            std::string name;
            std::vector<TestCase const*> unit_tests;
        };
    ...
    clara::Parser makeCommandLineParser( ConfigData& config ) {
        ...
        auto cli
            ...
            | Opt( config.showSuccessfulTests )
                ["-s"]["--success"]
                ( "include successful unit_tests in output" )
            | Opt( config.shouldDebugBreak )
                ["-b"]["--break"]
                ( "break into debugger on failure" )
            | Opt( config.noThrow )
                ["-e"]["--nothrow"]
                ( "skip exception unit_tests" )
			...
			| Arg( config.testsOrTags, "test name|pattern|tags" )
                ( "which test or unit_tests to use" );
...
namespace Catch {
    ...
    namespace {
            ...
            , m_context{config, makeReporter(config)}
            {
                ...
                if (m_matches.empty() && invalidArgs.empty()) {
                    ...
                    for (auto const& match : m_matches)
                        m_tests.insert(match.unit_tests.begin(), match.unit_tests.end());
                }
            }
        ...
        void applyFilenamesAsTags(Catch::IConfig const& config) {
            auto& unit_tests = const_cast<std::vector<TestCase>&>(getAllTestCasesSorted(config));
            for (auto& testCase : unit_tests) {
    ...
    int Session::runInternal() {
        ...
        CATCH_TRY {
            ...
            TestGroup unit_tests { m_config };
            auto const totals = unit_tests.execute();
...
namespace Catch {
    ...
    namespace Detail { namespace {
        ...
        public:
            ...
            // cout may get redirected when running unit_tests
...
// - white: No unit_tests ran.
// -   red: Failed [both/all] N test cases, failed [both/all] M assertions.
// - white: Passed [both/all] N test cases (no assertions).
// -   red: Failed N unit_tests cases, failed M assertions.
// - green: Passed [both/all] N unit_tests cases with M assertions.
void printTotals(std::ostream& out, const Totals& totals) {
    if (totals.testCases.total() == 0) {
        out << "No unit_tests ran.";
...
void ConsoleReporter::printTotals( Totals const& totals ) {
    if (totals.testCases.total() == 0) {
        stream << Colour(Colour::Warning) << "No unit_tests ran\n";
    } else if (totals.assertions.total() > 0 && totals.testCases.allPassed()) {
        stream << Colour(Colour::ResultSuccess) << "All unit_tests passed";
        stream << " ("
...
namespace Catch {
    ...
    void JunitReporter::writeGroup( TestGroupNode const& groupNode, double suiteTime ) {
        ...
        xml.writeAttribute( "unit_tests", stats.totals.assertions.total() );
